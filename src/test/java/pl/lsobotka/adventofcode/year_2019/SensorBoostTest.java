...
class SensorBoostTest extends BaseTest {

    private static Stream<Arguments> sensorBoost() {
        return Stream.of(Arguments.of(new long[] { 1102, 34915192, 34915192, 7, 4, 7, 99, 0 }, 0L, 1219070632396864L),
                Arguments.of(new long[] { 104, 1125899906842624L, 99 }, 0, 1125899906842624L));
    }

    @ParameterizedTest
    @MethodSource("sensorBoost")
    void sensorBoostTest(long[] program, long input, long expected) {
    ...
    private static Stream<Arguments> sensorBoostExample() {
        return Stream.of(
                Arguments.of(new long[] { 109, 1, 204, -1, 1001, 100, 1, 100, 1008, 100, 16, 101, 1006, 101, 0, 99 },
                        0L));
    }

    @ParameterizedTest
    @MethodSource("sensorBoostExample")
    void sensorBoostExampleTest(long[] program, long input) {
        SensorBoost boost = new SensorBoost(program, input);
        List<Long> actual = boost.execute();
    ...
    private static Stream<Arguments> sensorBoostFile() {
        return Stream.of(Arguments.of("2019/SensorBoost", 1L, 2518058886L),
                Arguments.of("2019/SensorBoost", 2L, 44292L));
    }

    @ParameterizedTest
    @MethodSource("sensorBoostFile")
    void sensorBoostFileTest(String fileName, long input, long expected) {
