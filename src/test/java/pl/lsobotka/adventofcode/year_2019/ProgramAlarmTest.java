...
class ProgramAlarmTest extends BaseTest {
    ...
    private static Stream<Arguments> simplePrograms() {
        return Stream.of(Arguments.of(new int[] { 1, 9, 10, 3, 2, 3, 11, 0, 99, 30, 40, 50 },
    ...
    void simpleProgramsTest(int[] program, int[] expected) {
        int[] actual = ProgramAlarm.processProgram(program);
        assertArrayEquals(expected, actual);
    }

    private static Stream<Arguments> programAlarm() {
        return Stream.of(Arguments.of("2019/ProgramAlarm", 12, 2, 4576384));
    }

    @ParameterizedTest
    @MethodSource("programAlarm")
    void programAlarmTest(String fileName, int firstVal, int secondVal, int expected) {
        int[] ints = getFileInput(fileName).stream()
                .map(s -> s.split(","))
                .flatMap(Stream::of)
    ...
    private static Stream<Arguments> findParams() {
        return Stream.of(Arguments.of("2019/ProgramAlarm", 19690720, 5398));
    }

    @ParameterizedTest
    @MethodSource("findParams")
    void findParamsTest(String fileName, int searchVal, int expected) {
        int[] ints = getFileInput(fileName).stream()
                .map(s -> s.split(","))
                .flatMap(Stream::of)
                .mapToInt(Integer::valueOf)
