...
public class GearRatios {
    ...
    int sumOfPEnginePartNumbers() {
        return schema.enginPartNumbers().stream().mapToInt(Integer::intValue).sum();
    }
    ...
    record Schema(Map<Coord, Character> schemaMap, List<Digit> digits, List<Gear> gears) {
        static Schema of(final List<String> input) {
            Map<Coord, Character> schemaMap = new HashMap<>();
        ...
        public List<Integer> enginPartNumbers() {
    ...
    public record Digit(int digit, List<Coord> coords) {
    ...
    record Gear(Coord coord, List<Digit> digits) {
        ...
        public int getRatio() {
            return digits.stream().map(Digit::digit).reduce((a, b) -> a * b).orElse(0);
        }
    }
}
